            ...
            {otherPerson[0].name ? (
              <>
                ...
                <div className="contact-img-container">
                  <img src={imagePath + otherPerson[0].image} />{" "}
                </div>
                {otherPerson[0].name}
              </>
            ) : (
            ...
            chat.messages.slice(-50).map((message) => (
              <div key={message._id}>
                <div className="msg-time">
                  <DateTimeConverter datetime={message.timestamp} /> mensaje de{" "}
                  {message.sender.name}
                </div>
                <div className="flex-div">
                  <span
                    className={
                      message.sender._id === you._id
                        ? "my-msg-container"
                        : "msg-container"
                    }
                  >
            ...
            {socketMessages.length < 1 ? (
              ...
              socketMessages.map((message, index) => (
                <>
                  <div key={index}>
                    <div className="msg-time">
                      <DateTimeConverter datetime={message.timestamp} />{" "}
                      {message.senderName}:
                    </div>
                    <div className="flex-div">
                      <span
                        className={
                          message.sender === you._id
                            ? "my-msg-container"
                            : "msg-container"
                        }
                      >
